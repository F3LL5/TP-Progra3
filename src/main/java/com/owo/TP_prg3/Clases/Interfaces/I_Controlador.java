...
public interface I_Controlador<Dto, FormDto, Id> {
    
    // Operaciones de lectura y consulta
    ResponseEntity<Set<Dto>> obtenerTodos();
    ResponseEntity<Dto> buscarPorID(Id id);
    ...
    ResponseEntity<Boolean> actualizar(Id id, FormDto dto);
    ResponseEntity<Boolean> eliminar(Id id);
}